...
compile_gnupg() {
        ...
        then
                [ ! -f "${GNUPG_SRCTAR}" ] &&
                        gen_die "Could not find gnupg source tarball: ${GNUPG_SRCTAR}. Please place it there, or place another version, changing /etc/genkernel.conf as necessary!"
                cd "${TEMP}"
                rm -rf "${GNUPG_DIR}"
                tar -zxpf "${GNUPG_SRCTAR}"
                [ ! -d "${GNUPG_DIR}" ] &&
                        gen_die "gnupg directory ${GNUPG_DIR} invalid"
                cd "${GNUPG_DIR}"
                print_info 1 'gnupg: >> Configuring...'
				export LDFLAGS="-static"
                ./configure --enable-gpg --enable-static >> ${LOGFILE} 2>&1 ||
                        gen_die 'Configuring gnupg failed!'
                print_info 1 'gnupg: >> Compiling...'
                make >> ${LOGFILE} 2>&1 ||
                        gen_die 'Compiling static gpg failed! Make sure you have gpg installed'
                cd "${TEMP}/${GNUPG_DIR}"
                print_info 1 'gnupg: >> Copying to cache...'
                [ -f "${TEMP}/${GNUPG_DIR}/g10/gpg2" ] ||
                        gen_die 'gpg executable does not exist!'
                strip "${TEMP}/${GNUPG_DIR}/g10/gpg2" ||
                        gen_die 'Could not strip gpg binaries!'
                mkdir "${TEMP}/gnupg"
                mkdir "${TEMP}/gnupg/bin"
                install -m 0755 -s /g10/gpg2 "${TEMP}/gnupg/bin/gpg"
                print_info 1 '      >> Copying to bincache...'
                cd "${TEMP}/gnupg"
                /bin/tar -cjf "${GNUPG_BINCACHE}" bin/ ||
                        gen_die 'Could not create binary cache'
                cd "${TEMP}"
                rm -rf "${TEMP}/gnupg" > /dev/null
