...
# Copyright(C) 2005 Gentoo Linux Installer Project
#
...
import sys, os, tempfile, random, string, re, types, exceptions
...
def _find_in_path(prog_name):
	...
	try:
		...
		for r_dir in string.split(PATH, ":"):
			file_path = os.path.join(r_dir, prog_name)
...
def _create_temporary_directory():
	...
	for tmpf in range(find_temporary_nb_attempts):
		try:
			# Using something >= 2**31 causes an error in Python 2.2...
			tmp_dir = os.path.join(tempfile.gettempdir(), "%s-%u" % ("pythondialog", random.randint(0, 2**30-1)))
		except os.error, v:
			raise PythonDialogOSError(v.strerror)
...
class Dialog:
	...
	def __init__(self, dialog="dialog", DIALOGRC=None, compat="dialog", use_stdout=None):
	...
	def _call_program(self, redirect_child_stdin, cmdargs, **kwargs):
		...
		if child_pid == 0:
			...
			try:
				...
				arglist = [self._dialog_prg] + self.dialog_persistent_arglist + _compute_common_args(kwargs) + cmdargs
	...
	def calendar(self, text, height=6, width=0, day=0, month=0, year=0, **kwargs):
		...
		"""
		(code, output) = self._perform( *(["--calendar", text, str(height), str(width), str(day), str(month), str(year)],), **kwargs)
	...
	def form(self, formtitle, fields, height=25, formheight=10, **kwargs):
		...
		Typical usage:
			data = d.form('title',(('Interface',5,'eth0'),('IP Address', 15), ('Netmask', 15)))
		"""
		...
		#Set the starting args FIXME: Do a real calc of the second arg
		formargs = ["--form", formtitle, str(height) , maxinputlen + maxoptlen + 12, str(formheight)]
		...
		for field in fields:
			...
			formargs.append(field[1])
	
			counter = counter + 1
		
		#Run the dialog program 
		for x in range(0, len(formargs)):
			formargs[x] = str(formargs[x])
	...
	"""
		(code, output) = self._perform(*(["--fselect", filepath, str(height), str(width)],),**kwargs)
	...
	def gauge_start(self, text="", height=8, width=54, percent=0, **kwargs):
		...
		"""
		(child_pid, child_rfd, child_stdin_wfd) = self._call_program(True,*(["--gauge", text, str(height), str(width), str(percent)],), **kwargs)
		try:
			self._gauge_process = {
				"pid": child_pid,
	...
	"""
		return self._perform(*(["--infobox", text, str(height), str(width)],),**kwargs)[0]
	...
	def inputbox(self, text, height=10, width=30, init='', **kwargs):
		...
		"""
		(code, tag) = self._perform(*(["--inputbox", text, str(height), str(width), init],),**kwargs)
	...
	"""
		return self._perform(*(["--msgbox", text, str(height), str(width)],), **kwargs)[0]
	...
	def passwordbox(self, text, height=10, width=60, init='', **kwargs):
		...
		"""
		(code, password) = self._perform(*(["--passwordbox", text, str(height), str(width), init],),**kwargs)
	...
	def scrollbox(self, text, height=20, width=78, **kwargs):
		...
		try:
			...
			try:
				...
				return self._perform(*(["--textbox", fName, str(height), str(width)],),	**kwargs)[0]
			finally:
	...
	def tailbox(self, filename, height=20, width=60, **kwargs):
		...
		"""
		return self._perform(*(["--tailbox", filename, str(height), str(width)],),**kwargs)[0]
	...
	def textbox(self, filename, height=20, width=60, **kwargs):
		...
		return self._perform(*(["--textbox", filename, str(height), str(width)],),**kwargs)[0]
	...
	def timebox(self, text, height=3, width=30, hour=-1, minute=-1,
		...
		"""
		(code, output) = self._perform(*(["--timebox", text, str(height), str(width), str(hour), str(minute), str(second)],),**kwargs)
		if code == self.DIALOG_OK:
			try:
	...
	def yesno(self, text, height=10, width=30, **kwargs):
		...
		"""
		return self._perform(*(["--yesno", text, str(height), str(width)],),**kwargs)[0]
