...
#				echo "#define UseDeprecatedKeyboardDriver YES" >> ${HOSTCONF}
		...
		if use minimal; then
			...
			if use x86; then
				...
				echo "#define XF86CardDrivers vmware vesa vga \
				      glint tga s3 s3virge rendition neomagic i740 cirrus tseng \ 
				      ark cyrix nsc i810 ati DevelDrivers via tseng trident chips apm \
				      XF86OSCardDrivers XF86ExtraCardDrivers" >> ${HOSTCONF}
			# (#93339)
			elif use sparc; then
